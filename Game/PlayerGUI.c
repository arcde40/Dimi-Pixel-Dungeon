...
void generateInventory(Player* p, wchar_t defaultBuffer[][189], WORD colorMap[][189], int cursorPosition, int buttonCurPosition) {
	...
	for (int i = 0; i < p->inventory->size; i++) {
		if (i < p->inventory->size) {
			...
			for (int j = offset; str[j-offset] != NULL; j++) {
				...
				if(!(item.metadata & ITEM_IDENTIFIED) && (item.metadata & ITEM_IDENTIFIABLE)) colorMap[8 + i * 2][j] = COLOR_MAGENTA;
	...
	if (cursorPosition >= 0 && cursorPosition < p->inventory->size) {
		...
		wchar_t str[1000] = { 0, };
		char temp[1000] = { 0, };
		if (item.ITEM_TYPE == ITEM_WEAPON) sprintf(temp, "%s|||이 무기는 &e%d레벨 무기&f이며,|&e%d - %d 데미지&f를 입힐 수 있습니다.|%s%s%s%s%s||%s", item.lore, item.level, item.minDamage, item.maxDamage, (item.accuarcy < DEFAULT_ACCURACY) ? "이 무기는 정확도가 떨어집니다.|" : "", (item.armor > 0) ? "이 무기는 일정 피해를 흡수할 수 있습니다.|" : "", (item.attackRange > 1)? "이 무기는 멀리 떨어진 적을 공격할 수 있습니다.|" : "", (item.attackSpeed < 1) ? "이 무기는 조금 더 빠르게 공격할 수 있습니다.|":"", (!(item.metadata & ITEM_IDENTIFIED) ? "" : ""), ((p->level < item.level) ? "&c이 무기의 레벨이 당신의 레벨보다 높기 때문에 장착하였을 시 공격 속도와 정확도 모두 감소합니다." : ""));
		else if(item.ITEM_TYPE == ITEM_ARMOR) sprintf(temp, "%s|||이 방어구는 &e%d레벨 방어구&f이며,|&e%d 데미지&f를 방어 할 수 있습니다.|%s%s||%s", item.lore, item.level, item.armor, "", "", ((p->level < item.level) ? "&c이 방어구의 레벨이 당신의 레벨보다 높기 때문에 장착하였을 시 이동 속도와 회피율 모두 감소합니다." : ""));
