...
void playerAttack(Player* p, MobInfo* mob, MobList* list, LogArrayList* log) {
	if (p->debuff[3] >= 0) p->debuff[3] = -1;
	int accuracy = p->accuracyConstant * p->equippedWeapon->accuarcy;
	if (p->equippedWeapon->level - p->level > 0) accuracy /= (p->equippedWeapon->level - p->level) + 10;
	int chance = rand() % (accuracy + mob->dodgeConstant);
	if (chance > accuracy) {
		char t[100] = { 0, };
		sprintf(t, "%s이(가) 공격을 회피하였다!", mob->name);
		putMBS(log, t);
	...
	if (mob->Health <= 0) {
		...
		int exp = mob->maxHealth / mob->minDamage + 1;
		p->exp += exp;
		char t[100] = { 0, };
		sprintf(t, "%s에게 %d 데미지를 입혀 제거하였다! (%d Exp)", mob->name, damage, exp);
		putMBS(log, t);
		if (p->exp >= MaxExp(p->level)) {
			p->exp -= MaxExp(p->level);
			p->level += 1;
			char tt[100] = { 0, };
			sprintf(tt, "레벨 업! 체력과 명중률, 회피율이 증가합니다.", mob->name);
			p->Health += p->level * 5;
			p->maxHealth += p->level * 3;
			putMBS(log, tt);
		}
	}
	else {
